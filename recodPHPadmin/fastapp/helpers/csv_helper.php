...
function download_csv($output = NULL,$filename = NULL) {
...
function generate_csv($rowData) {
    ...
    foreach ($rowData as $row) {
        $schema_insert = '';
        for ($j = 0; $j < $fields_cnt; $j++) {
            if (isset($row[$fields[$j]]) && ($row[$fields[$j]] == '0' || $row[$fields[$j]] != '')) {
                if ($csv_enclosed == '') {
                    $schema_insert .= $row[$fields[$j]];
                } else {
                    $schema_insert .= $csv_enclosed .
                            str_replace($csv_enclosed, $csv_escaped . $csv_enclosed, $row[$fields[$j]]) . $csv_enclosed;
                }
            } else {
                $schema_insert .= '';
            }
